...
// GET FavoriteLists by customer
router.get('/:customerName', jwtAuthentication, async (req, res) => {
    try {
...
router.post('/', async (req, res) => {
    ...
    if (!selectedList || !Array.isArray(selectedList) || selectedList.length === 0) {
        return res.status(400).json({ message: 'Selected list is required and must be a non-empty array' });
    }
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const deletedFavoriteList = await FavoriteList.findByIdAndDelete(favoriteListId);
